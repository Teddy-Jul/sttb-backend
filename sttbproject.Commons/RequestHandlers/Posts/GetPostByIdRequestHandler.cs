...
public class GetPostByIdRequestHandler : IRequestHandler<GetPostByIdRequest, PostDetailResponse>
{
    ...
    public async Task<PostDetailResponse> Handle(GetPostByIdRequest request, CancellationToken cancellationToken)
    {
        var post = await _context.Posts
            .Include(p => p.Author)
            .Include(p => p.Categories)
            .Include(p => p.FeaturedImage) // Also include image
            .FirstOrDefaultAsync(p => p.PostId == request.PostId, cancellationToken);

        if (post == null)
        {
            _logger.LogWarning("Post not found with ID: {PostId}", request.PostId);
