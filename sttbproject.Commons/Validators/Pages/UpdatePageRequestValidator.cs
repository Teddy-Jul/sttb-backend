...
public class UpdatePageRequestValidator : AbstractValidator<UpdatePageRequest>
{
    ...
    public UpdatePageRequestValidator(SttbprojectContext context)
    {
        _context = context;
        ...
        RuleFor(x => x.UpdatedBy)
            .GreaterThan(0).WithMessage("Invalid updater ID");
    ...
    private async Task<bool> BeUniqueSlug(UpdatePageRequest request, string slug, CancellationToken cancellationToken)
    {
        return !await _context.Pages.AnyAsync(p => p.Slug == slug && p.PageId != request.PageId, cancellationToken);
    }
